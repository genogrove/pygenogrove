...
def test_roundtrip_intervals(tmp_path):
    ...
    g.insert("chr2", pg.GenomicCoordinate(".", 150, 250))
    ...
    hits = loaded.intersect(pg.GenomicCoordinate(".", 150, 350), "chr1")
    assert len(hits) == 2
...
def test_roundtrip_preserves_edges(tmp_path):
    ...
    src = list(loaded.intersect(pg.GenomicCoordinate(".", 100, 200), "chr1"))[0]
...
def test_roundtrip_edge_chain_across_splits(tmp_path):
    ...
    g = pg.Grove(3)
    e1 = g.insert("chr1", pg.GenomicCoordinate(".", 1000, 1200))
    e2 = g.insert("chr1", pg.GenomicCoordinate(".", 2000, 2200))
    e3 = g.insert("chr1", pg.GenomicCoordinate(".", 3000, 3200))
    ext = g.add_external_key(pg.GenomicCoordinate(".", 5000, 5500))
    g.add_edge(e1, e2)
    g.add_edge(e2, e3)
    g.add_edge(e3, ext)
    ...
    keys = list(loaded.intersect(pg.GenomicCoordinate(".", 1000, 3200), "chr1"))
    assert len(keys) == 3
