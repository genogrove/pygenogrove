...
def test_remove_single_key_from_root_leaf():
    ...
    assert len(g.intersect(pg.GenomicCoordinate(".", 100, 200), "chr1")) == 0
...
def test_remove_all_keys_empties_index():
    pg = _pg()
    g = pg.Grove(4)
    keys = [g.insert("chr1", pg.GenomicCoordinate(".", i * 10, i * 10 + 5)) for i in range(3)]
    for k in keys:
        assert g.remove_key("chr1", k) is True
    assert g.indexed_vertex_count() == 0
    assert len(g.intersect(pg.GenomicCoordinate(".", 0, 1000), "chr1")) == 0
...
def test_remove_multi_index_independence():
    ...
    pg = _pg()
    g = pg.Grove(4)
    chr1 = [g.insert("chr1", pg.GenomicCoordinate(".", i * 10, i * 10 + 5)) for i in range(5)]
    [g.insert("chr2", pg.GenomicCoordinate(".", i * 10, i * 10 + 5)) for i in range(5)]

    assert g.remove_key("chr1", chr1[2]) is True
    assert len(g.intersect(pg.GenomicCoordinate(".", 0, 1000), "chr1")) == 4
    assert len(g.intersect(pg.GenomicCoordinate(".", 0, 1000), "chr2")) == 5
...
def test_stress_remove_with_rebalancing():
    ...
    pg = _pg()
    g = pg.Grove(4)
    keys = [g.insert("chr1", pg.GenomicCoordinate(".", i * 10, i * 10 + 5)) for i in range(40)]
    ...
    res = g.intersect(pg.GenomicCoordinate(".", 0, 100000), "chr1")
    assert len(res) == 15
...
def test_vertex_counts():
    ...
    g.add_external_key(pg.GenomicCoordinate(".", 500, 600))
...
def test_compact_preserves_keys_without_removal():
    ...
    for i in range(20):
        assert len(g.intersect(pg.GenomicCoordinate(".", i * 10, i * 10 + 5), "chr1")) == 1


def test_compact_reclaims_storage_after_removals():
    pg = _pg()
    g = pg.Grove(4)
    keys = [g.insert("chr1", pg.GenomicCoordinate(".", i * 10, i * 10 + 5)) for i in range(20)]
    for k in keys[:10]:
        assert g.remove_key("chr1", k) is True
...
def test_compact_graph_edges_survive():
    ...
    pg = _pg()
    g = pg.Grove(4)
    keys = [g.insert("chr1", pg.GenomicCoordinate(".", i * 10, i * 10 + 5)) for i in range(10)]
    ...
    src = list(g.intersect(pg.GenomicCoordinate(".", 20, 25), "chr1"))[0]
    tgt = list(g.intersect(pg.GenomicCoordinate(".", 30, 35), "chr1"))[0]
...
def test_compact_external_keys_and_cross_edges_preserved():
    ...
    pg = _pg()
    g = pg.Grove(4)
    indexed = g.insert("chr1", pg.GenomicCoordinate(".", 0, 5))
    external = g.add_external_key(pg.GenomicCoordinate(".", 1000, 1100))
    g.add_edge(indexed, external)   # indexed -> external
    g.add_edge(external, indexed)   # external -> indexed

    victim = g.insert("chr1", pg.GenomicCoordinate(".", 10, 15))
    assert g.remove_key("chr1", victim) is True

    g.compact()

    # External key handle is still valid after compact().
    assert external.value == pg.GenomicCoordinate(".", 1000, 1100)
    assert g.external_vertex_count() == 1

    # The indexed endpoint moved; re-discover it and check both edge directions.
    new_indexed = list(g.intersect(pg.GenomicCoordinate(".", 0, 5), "chr1"))[0]
...
def test_compact_roundtrip_serialization(tmp_path):
    ...
    res = loaded.intersect(pg.GenomicCoordinate(".", 0, 100000), "chr1")
    assert sorted(k.value.start for k in res) == [i * 10 for i in range(10, 20)]
...
def test_remove_key_on_data_grove():
    pg = _pg()
    g = pg.BedGrove(4)
    a = g.insert("chr1", pg.GenomicCoordinate(".", 100, 200), pg.BedEntry("chr1", 100, 201))
